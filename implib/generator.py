...
class Generator:
    ...
    def run(
        ...
        all_symbols = self.backend.collect_symbols(input_path)

        demangled: dict[str, str] = {s.name: s.demangled for s in all_symbols if s.demangled}

        def is_exported(s: Symbol) -> bool:
            if not s.exported:
                return False
            # Standard ELF/Mach-O filters
            if s.bind == "LOCAL" or s.typ == "NOTYPE" or s.ndx == "UND" or not s.default:
                return False
            # Filter special entry/exit points
            name = s.name.lstrip('_')
        ...
        syms_list = [s for s in all_symbols if is_exported(s)]
        ...
        if exported_data:
            warn(
                ...
                + ", ".join(name.lstrip('_') for name in exported_data)
            )
