...
from implib.base import BinaryBackend, BackendError, MAGIC_ELF, Symbol, SectionInfo, RelocationInfo
...
class ElfBackend(BinaryBackend):
    ...
    def collect_symbols(self) -> list[Symbol]:
        ...
        func_sizes = {f.address: f.size for f in elf.functions if f.size > 0}

        def get_exact_size(sym: lief.ELF.Symbol) -> int:
            if sym.size > 0: return sym.size
            if not sym.value: return 0
            
            if sym.value in func_sizes:
                return func_sizes[sym.value]
            
            next_sym_addr = next_addr_map.get(sym.value)
            sec = elf.section_from_virtual_address(sym.value)
            ...
            return max(0, limit - sym.value) if limit else 0
    ...
    def default_load_name(self) -> str:
        ...
        for entry in self.binary.dynamic_entries:
            if int(entry.tag) == 14: # 14 is DT_SONAME
                return entry.name
