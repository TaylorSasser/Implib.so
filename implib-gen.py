            ...
            with open(self.path, "r") as f:
                ...
                for line in (l.split(';')[0].strip() for l in f if l.split(';')[0].strip()):
                    if line.upper() == "EXPORTS":
                        exports_found = True
                    elif exports_found and re.match(r"^([A-Za-z0-9_]+)$", line):
                        out.append(Symbol(line, "GLOBAL", "FUNC", "1", demangled=line))
        ...
        if self.binary:
            try:
                for cmd in self.binary.commands:
                    if cmd.command == lief.MachO.LoadCommand.TYPE.ID_DYLIB: return cmd.name if cmd.name.startswith(
                        ("@", "/")) else os.path.basename(cmd.name)
            except Exception:
                pass
        return os.path.basename(self.path)
