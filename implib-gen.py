...
class Symbol:
    ...
    # Pre-calculated score to avoid tuple allocation on every comparison access
    _score: tuple[bool, bool, bool, bool, bool] = field(init=False, repr=False)
...
class BinaryBackend(ABC):
    ...
    _bin: Any
    _loaded: bool
    _next_addr_map: dict[int, int] | None
    _func_sizes: dict[int, int] | None
    ...
    def __init__(self, path: str):
        ...
        self._bin = None
        self._loaded = False
        self._next_addr_map = None
        self._func_sizes = None
    ...
    def _get_exact_size(self, val: int, original_size: int = 0) -> int:
        ...
        if not val or not self.binary:
            return 0

        if self._func_sizes is None:
            # Optimize list duplication out by using islice
            all_addrs = sorted({sym.value for sym in self.binary.symbols if sym.value != 0})
            self._next_addr_map = dict(zip(all_addrs, itertools.islice(all_addrs, 1, None)))
            self._func_sizes = {f.address: f.size for f in self.binary.functions if f.size > 0}

        if val in self._func_sizes:
            return self._func_sizes[val]

        assert self._next_addr_map is not None
...
class ElfBackend(BinaryBackend):
    ...
    @property
    def binary(self) -> lief.ELF.Binary | None:
        return super().binary
    ...
    def collect_symbols(self) -> list[Symbol]:
        ...
        for sym in itertools.chain(self.binary.dynamic_symbols, self.binary.symbols):
            ...
            ndx = (
                "UND" if sym.imported or sym.shndx == 0 else
                "ABS" if sym.shndx == 0xFFF1 else
                "COM" if sym.shndx == 0xFFF2 else
                str(sym.shndx)
            )
            ...
            s_obj = Symbol(
                name=sym.name, bind=sym.binding.name, typ=sym.type.name, ndx=ndx,
                value=sym.value, size=self._get_exact_size(sym.value, sym.size),
                default=default, version=ver_name, demangled=sym.demangled_name or sym.name
            )
            ...
            if not existing or s_obj.score > existing.score or (existing.version is None and s_obj.version is not None):
                if existing and s_obj.version is None:
    ...
    def collect_relocations(self) -> list[RelocationInfo]:
        ...
        addr_syms = sorted([
            ...
            for s in self.binary.symbols if s.name and s.value != 0 and not s.name.startswith(".")
        ], key=lambda t: t[0])
...
def _collect_relocated_data(all_syms: list[Symbol], syms: dict[str, Symbol], bites: dict[str, bytes],
    ...
    for name, s in sorted(syms.items()):
        ...
        entries: list[tuple[str, Any]] = [
            ("offset", int.from_bytes(b[i:i + ptr_size], byteorder=byteorder, signed=False))
            for i in range(0, len(b), ptr_size)
        ]

        for rel in rels:
            if rel.typ in reloc_types and s.value <= rel.offset < s.value + s.size:
                i = (rel.offset - s.value) // ptr_size
                if i < len(entries):
...
def _generate_vtables(cls_syms: dict[str, Symbol], cls_data: dict) -> str:
    ...
    for name, s in sorted(cls_syms.items()):
        ...
        if data and data[0][0] == "byte":
            ...
            fields = " ".join(f"{c_types[typ]} field_{i};" for i, (typ, _) in enumerate(data))
            declarator = f"const struct {{ {fields} }} %s"
...
class Generator:
    ...
    def run(self, *, input_path: str, outdir: str, stem: str, load_name: str, funs_allowlist: list[str] | None,
        ...
        for s in all_exported_symbols:
            ...
            if not s.default:
                if s.typ == "FUNC":
                    ...
                    if opts.verbose:
                        self.info(f"Skipping hidden/versioned symbol {s.name}")
                continue
        ...
        exported_data = [
            s.name for s in syms_list
            if (s.typ in ("OBJECT", "COMMON", "TLS") or s.ndx == "COM")
               and (not opts.vtables or not is_vtable_name(s.name, demangled.get(s.name, "")))
        ]
        ...
        if opts.vtables:
            ...
            if cls_syms:
                vtable_text = _generate_vtables(
                    cls_syms,
                    _collect_relocated_data(
                        all_exported_symbols, cls_syms,
                        _read_unrelocated_data(self.backend, cls_syms),
                        self.backend.collect_relocations(),
                        opts.ptr_size, opts.symbol_reloc_types,
        ...
        with open(os.path.join(outdir, tramp_file), "w") as f:
            f.write(string.Template((self.templates_dir / "table.S.tpl").read_text()).substitute(
            ...
            for i, name in enumerate(funs):
                f.write(tramp_tpl.substitute(
                    lib_suffix=lib_suffix, sym=opts.symbol_prefix + name,
                    offset=i * opts.ptr_size, number=i
                ))
        ...
        with open(os.path.join(outdir, init_file), "w") as f:
            f.write(string.Template((self.common_dir / "init.c.tpl").read_text()).substitute(
                ...
                no_dlopen=int(not opts.dlopen), lazy_load=int(opts.lazy_load),
...
def main(argv: list[str] | None = None) -> int:
    ...
    opts = GenOptions(
        ...
        args.symbol_prefix, args.dlopen_callback, args.dlsym_callback,
        int(cfg["Arch"]["PointerSize"]), set(re.split(r"\s*,\s*", cfg["Arch"]["SymbolReloc"]))
    )
    ...
    Generator(
        backend,
        templates_dir=str(platform_root / arch),
